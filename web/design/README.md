...
Sampled directly from the logo ribbon — orchid purple → deep indigo-violet crossing → azure blue — so it reads as the mark, not a generic purple-blue. Use it for:
...
Three calibrated intensities share the same logo hues (all in `colors_and_type.css`):

- `--ocx-gradient` — full-strength 135° ramp. Small punchy fills: CTAs, the app icon, avatar fills.
- `--ocx-gradient-dark` — deep indigo field with orchid + azure halos. Dark hero surfaces; white text.
- `--ocx-gradient-light` — airy lavender→ice-blue wash with whisper glows. Light marketing blocks; dark text.
...
The system is theme-aware at the **semantic-token layer**. Light is the default (`:root`); dark applies via `[data-theme="dark"]` (explicit toggle) or `prefers-color-scheme: dark` (auto, unless light is pinned). Dark re-maps only the semantic tokens — `--bg*`, `--fg*`, `--border*`, shadows, dark-blue tints, and the component tokens (`--surface`, `--field-bg`, `--btn-primary-bg`). The raw brand colors never change. Dark surfaces are indigo-tinted near-black built from the brand's own `#15104f` family — not flat black. Any component built on the semantic tokens adapts automatically.
